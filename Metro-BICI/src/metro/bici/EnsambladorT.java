 ...
 */
public class EnsambladorT extends Thread {
    ...
    public void run(){
        ...
        while(run){
            if (run && Integer.parseInt(this.num.getText())<(Integer.parseInt(txt.getVec()[2]))){
            
            try {
                int aux1=Integer.parseInt(dia.getText())+3;
                //dia.setText(Integer.toString(aux1));
                Ensamblador1.append("---------- Inicio de Ensamblaje de triciclo ----------- \n");
                System.out.println("------->"+ConsumRuedas.availablePermits());
                
                ConsumRuedas.acquire(3);
                exR.acquire();
                BarraR.setValue(ConsumRuedas.availablePermits());
                BarraR.setString(" #Ruedas = "+(ConsumRuedas.availablePermits()));
                exR.release();
                ProductRuedas.release(3);
               ...
               //Ensamblador1.append("Ha tomado 1 marco \n");
                ...
                Ensamblador1.append("Ha armado un triciclo baby \n");
             
                Ensamblador1.append("-------- Se ha creado La Metro-trici ----------- \n");
                int aux=Integer.parseInt(cont.getText())+1;
                cont.setText(Integer.toString(aux));
                
                EnsambladorT.sleep(this.tiempo);
    ...
    public int getTriciclos() {
        return triciclos;
    }

    public void setTriciclos(int triciclos) {
        this.triciclos = triciclos;
    }
